DECLARE @workCalendarCode INT = 1;

IF EXISTS (SELECT PUBLISHED_ARTIFACT_ID FROM ORG.WORK_CALENDAR
WHERE WORK_CALENDAR_CODE = @workCalendarCode)
	BEGIN
		DECLARE @publishedArtifactId NVARCHAR(max) = (SELECT PUBLISHED_ARTIFACT_ID FROM ORG.WORK_CALENDAR
		WHERE WORK_CALENDAR_CODE = @workCalendarCode);

		DECLARE @entityId NVARCHAR(max) = (SELECT ENTITY_ID FROM BFX.ENTITY_REF
		WHERE PUBLISHED_ARTIFACT_ID = @publishedArtifactId);

		DELETE FROM ORG.WORK_CALENDAR
		WHERE PUBLISHED_ARTIFACT_ID = @publishedArtifactId

		DELETE FROM BFX.RECENT_DOCUMENT
		WHERE ENTITY_REF_ID = @entityId
...
IF NOT EXISTS (SELECT PUBLISHED_ARTIFACT_ID FROM ORG.WORK_CALENDAR
WHERE WORK_CALENDAR_CODE = @workCalendarCode)
	BEGIN
		...
		IF @publishedArtifactId IS NOT NULL
			BEGIN
