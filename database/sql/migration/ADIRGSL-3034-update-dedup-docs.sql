-- ############### Check dedup information
--SELECT * FROM PTY_IMPL.PARTY_DEDUP_INFO
--SELECT * FROM PTY_IMPL.PARTY_DEDUP_DELETED

-- ############### Clean processed information
--UPDATE PTY_IMPL.PARTY_DEDUP_INFO
--SET UPDATE_ORDER = 0, IS_PROCESSED = 0, ERROR = NULL
--WHERE ID = 'ID'
...
SELECT pdi.ID, pdi.PARTY_CODE, p.PARTY_ID, pdi.DEDUPL_NUMBER, LOWER(d.PARTY_ID) DEDUPL_ID
...
fetch next FROM cur_party into @rowId, @partyCode, @partyId, @deduplCode, @deduplId;
...
BEGIN
	...
	-- PRINT '';
	-- PRINT N'Удаляем информацию старого контрагента ↓';
	-- SET STATISTICS TIME ON;
	DELETE FROM ORG.SERVICE_PROVIDER_HISTORY WHERE PARTY_CODE = @partyCode
	DELETE FROM ORG.SERVICE_PROVIDER WHERE PARTY_CODE = @partyCode
	DELETE FROM PTY.PARTY_HISTORY WHERE PARTY_CODE = @partyCode
	DELETE FROM PTY.PERSON_HUB WHERE PERSON_CODE = @partyCode
	DELETE pis FROM PTY_IMPL.PARTY_INFO_SAT pis
	LEFT JOIN PTY_IMPL.PARTY_HUB ph ON ph.PARTY_HKEY = pis.PARTY_INFO_HKEY
	WHERE ph.PARTY_CODE = @partyCode
	DELETE FROM PTY_IMPL.PARTY_HUB WHERE PARTY_CODE = @partyCode
	DELETE FROM PTY.PARTY WHERE PARTY_CODE = @partyCode
	-- SET STATISTICS TIME OFF;
	...
	BEGIN CATCH
		...
		-- SET STATISTICS TIME ON;
		UPDATE PTY_IMPL.PARTY_DEDUP_INFO
		SET ERROR = CONCAT('ErrorMessage:',@ErrorMessage, ' ErrorSeverity:', @ErrorSeverity, '. ErrorState:', @ErrorState, '.')
		WHERE PARTY_CODE = @partyCode and DEDUPL_NUMBER = @deduplCode
		-- SET STATISTICS TIME OFF;
