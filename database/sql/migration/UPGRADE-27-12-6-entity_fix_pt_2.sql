...
from bfx.entity_retention x,
     bfx.entity_retention z,
     pas.contract y
  ...
  (select q.retention_group_id
   from
     (select r.related_entity_id as entity_id,
      ...
      from bfx.entity_relation r
      join
        (select z.entity_id,
                z.retention_group_id
         from bfx.entity_retention z
         where z.retention_group_id in
             (select g.retention_group_id
              from bfx.entity_retention g
              where exists
                  (select 1
                   from pas.contract c
