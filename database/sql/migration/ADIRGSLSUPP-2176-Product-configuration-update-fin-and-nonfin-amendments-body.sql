...
SELECT
	...
	CASE
        WHEN ps.PRODUCT_CODE IS NOT NULL
            ...
            ELSE JSON_VALUE(c.BODY, '$.mainInsuranceConditions.insuranceProduct.productCode')
    END as PRODUCT_CODE,
	CASE
        WHEN ams.ISSUE_DATE IS NOT NULL
            THEN ams.ISSUE_DATE
            ELSE JSON_VALUE(am.BODY, '$.basicConditions.issueDate')
    END as ISSUE_DATE
INTO #CONTRACT_PRODUCT_CONF_TO_UPDATE
FROM PAS.CONTRACT am
LEFT JOIN PAS_IMPL.CONTRACT_BODY_PRODUCT_CONF_UPDATE_INFO cbui ON cbui.CONTRACT_NUMBER = am.CONTRACT_NUMBER
LEFT JOIN PAS.CONTRACT c ON c.CONTRACT_ID = am.ORIGINAL_DOCUMENT_ID
...
LEFT JOIN PAS_IMPL.AMENDMENT_HUB ah ON ah.AMENDMENT_NUMBER = am.CONTRACT_NUMBER
LEFT JOIN PAS_IMPL.AMENDMENT_SAT_LATEST ams ON ams.AMENDMENT_HKEY = ah.AMENDMENT_HKEY
LEFT JOIN CFX.PUBLISHED_ARTIFACT pa ON pa.PUBLISHED_ARTIFACT_ID = am.PUBLISHED_ARTIFACT_ID
WHERE cbui.IS_UPDATED IS NULL
AND (pa.CODE_NAME LIKE N'%FinChange' OR pa.CODE_NAME LIKE N'%NonFinChange')
