...
DECLARE @contractBodyData TABLE(
    QUOTE_BENEFICIARY_HKEY nvarchar(max) NOT NULL,
...
select DISTINCT quoteHub.QUOTE_HKEY AS QUOTE_BENEFICIARY_HKEY,
...
FROM PAS_IMPL.QUOTE_HUB quoteHub,
	PAS.CONTRACT contract
	CROSS APPLY OPENJSON(
		JSON_QUERY(contract.body, '$.beneficiaries.beneficiaries')
	) beneficiaries
WHERE contract.CONTRACT_NUMBER = quoteHub.CONTRACT_NUMBER

UPDATE PAS_IMPL.QUOTE_BENEFICIARY_SAT
SET BENEFICIARY_ID = cbd.BENEFICIARY_ID
FROM PAS_IMPL.QUOTE_BENEFICIARY_SAT sat, @contractBodyData cbd
WHERE cbd.QUOTE_BENEFICIARY_HKEY = sat.QUOTE_BENEFICIARY_HKEY
