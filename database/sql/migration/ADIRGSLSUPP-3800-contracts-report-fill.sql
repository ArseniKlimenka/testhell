...
with RankedRates as (
	select 
	       ph.contract_number, 
	       pcs.manual_rate as manual_rate,
	       pcs.calculated_rate as calculated_rate,
	       pcs.calculation_date as calculation_date,
	       row_number() over (partition by ph.contract_number order by pcs.calculation_date desc) as rn
	from pas_impl.policy_hub ph
	join pas_impl.policy_commission_link pcl on ph.policy_hkey = pcl.policy_hkey
	join pas_impl.policy_commission_sat pcs on pcl.policy_commission_hkey = pcs.policy_commission_hkey
	where pcs.period_number = 1
)
...
(
    ...
    HOLDER_CODE,
    INSURED_CODE,
    INITIATOR_EMPLOYEE_CODE,
    INITIATOR_ORGUNIT_CODE,
    IS_REINVEST,
    ISSUE_FORM_CODE,
    IS_PERSONAL_MANAGER,
    CREDIT_SUM,
...
select prod.product_group,
       ...
       sat.credit_sum,
       sat.credit_rate,
       sat.annuity_payment_sum,
       sat.credit_program_id,
       prod.sales_segment as product_sales_segment,
       sat.strategy_description_full,
       rr.calculated_rate,
       rr.manual_rate,
       spisl.tab_number
  from (
        select 1 is_policy,
               ...
               satl.credit_sum_net,
               satl.credit_rate,
               satl.annuity_payment_sum,
               satl.credit_program_id,
               satl.STRATEGY_DESCRIPTION_FULL
          from pas_impl.policy_hub hub
          join pas_impl.policy_sat_latest satl on satl.policy_hkey = hub.policy_hkey
          join pas.contract c on c.contract_number = hub.contract_number
       ...
       left join RankedRates rr on rr.contract_number = sat.contract_number and rr.rn = 1;
