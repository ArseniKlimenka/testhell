IF EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[UNI_IMPL].[RQT_HOLDER_LINK]') AND TYPE IN (N'U'))
BEGIN
DELETE FROM UNI_IMPL.RQT_HOLDER_LINK
END
GO
...
INSERT INTO UNI_IMPL.RQT_HOLDER_LINK (
	RQT_HOLDER_HKEY,
	LOAD_DATE,
	RECORD_SOURCE,
	RQT_HKEY,
...
LEFT JOIN PTY_IMPL.PARTY_HUB partyHub ON partyHub.PARTY_CODE = rqtSat.HOLDER_CODE
WHERE partyHub.PARTY_HKEY IS NOT NULL AND rqtHub.RQT_HKEY IS NOT NULL
AND rqtHub.RQT_HKEY NOT IN (SELECT RQT_HKEY FROM UNI_IMPL.RQT_HOLDER_LINK)
