...
SELECT DISTINCT
	...
	qsl.HOLDER_CODE SAT_HOLDER_CODE, 
	pdi.DEDUPL_NUMBER
...
FROM PAS.CONTRACT c
	...
	LEFT JOIN PTY_IMPL.PARTY_DEDUP_INFO pdi ON pdi.DEDUPL_NUMBER = qsl.HOLDER_CODE
WHERE 
	qsl.HOLDER_CODE != JSON_VALUE(c.BODY, '$.policyHolder.partyData.partyCode')
	AND pdi.DEDUPL_NUMBER = qsl.HOLDER_CODE
	AND c.SEQ_NUMBER = 0
...
SELECT DISTINCT
	...
	qsl.INSURED_CODE SAT_INSURED_CODE, 
	pdi.DEDUPL_NUMBER
INTO #CONTRACT_WITH_PARTY_ERR_INSURED
FROM PAS.CONTRACT c
	LEFT JOIN PAS_IMPL.QUOTE_HUB qs ON qs.CONTRACT_NUMBER = c.CONTRACT_NUMBER
	LEFT JOIN PAS_IMPL.QUOTE_SAT_LATEST qsl on qsl.QUOTE_HKEY = qs.QUOTE_HKEY
	LEFT JOIN PTY_IMPL.PARTY_DEDUP_INFO pdi ON pdi.DEDUPL_NUMBER = qsl.INSURED_CODE
WHERE 
	qsl.INSURED_CODE != JSON_VALUE(c.BODY, '$.insuredPerson.partyData.partyCode') 
	AND pdi.DEDUPL_NUMBER = qsl.INSURED_CODE
	AND c.SEQ_NUMBER = 0
...
SELECT DISTINCT
	...
	qsl.INITIATOR_EMPLOYEE_CODE SAT_INITIATOR_EMPLOYEE_CODE,
	pdi.DEDUPL_NUMBER
INTO #CONTRACT_WITH_PARTY_ERR_INITIATOR
FROM PAS.CONTRACT c
	LEFT JOIN PAS_IMPL.QUOTE_HUB qs ON qs.CONTRACT_NUMBER = c.CONTRACT_NUMBER
	LEFT JOIN PAS_IMPL.QUOTE_SAT_LATEST qsl on qsl.QUOTE_HKEY = qs.QUOTE_HKEY
	LEFT JOIN PTY_IMPL.PARTY_DEDUP_INFO pdi ON pdi.DEDUPL_NUMBER = qsl.INITIATOR_EMPLOYEE_CODE
WHERE 
	qsl.INITIATOR_EMPLOYEE_CODE != JSON_VALUE(c.BODY, '$.initiator.employeeCode')
	AND pdi.DEDUPL_NUMBER = qsl.INITIATOR_EMPLOYEE_CODE
	AND c.SEQ_NUMBER = 0
