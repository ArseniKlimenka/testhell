...
{
    public class EndowmentCalcServiceExecutor : IEndowmentCalcServiceExecutor
    {
        private readonly IIntegrationServiceExecutor _integrationServiceExecutor;

        public EndowmentCalcServiceExecutor(IIntegrationServiceExecutor integrationServiceExecutor)
        {
            _integrationServiceExecutor = integrationServiceExecutor;
        }

        public async Task<JsonObject> ExecuteCalculation(EndowmentCalculationRequest request)
        {
            ...
            using (new ApplicationContextImpersonation(new ImpersonationOptions(Guid.Empty, "System")))
            {
                ...
                response = await _integrationServiceExecutor.Execute("CalculateEndowmentAmounts", "1", serviceRequest);
            }
