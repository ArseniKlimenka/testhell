...
module.exports = function mapping(input) {
    ...
    const strategyCode = getValue(input.body, 'basicInvestmentParameters.investmentStrategy.investmentStrategyCode');
    const baseActiveDescription = getValue(input.body, 'basicInvestmentParameters.baseActiveDescription');
    const optionPrice = round(getValue(input.body, 'basicInvestmentParameters.optionPrice', 1) * 100, 2);
    const participationCoeff = round(getValue(input.body, 'basicInvestmentParameters.participationCoeff', 1) * 100, 2) + '%';
    const barrier = getValue(input.body, 'basicInvestmentParameters.barrier');
