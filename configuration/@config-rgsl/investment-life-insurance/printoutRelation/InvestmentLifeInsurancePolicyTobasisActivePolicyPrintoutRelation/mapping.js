...
module.exports = function mapping(input) {
    ...
    let surrenderValues = printoutsHelper.getSurrenderValues(body?.surrenderValues, coolOffDays);
    if (isBasisActive2VTB) {
        surrenderValues = printoutsHelper.getSurrenderValuesWithRisk(surrenderValues, body.risks, 'DLP36904');
    }
    ...
    if (isThreeRisks) {
        differenceDNS = printoutsHelper.getDifference2(body.risks, thirdRisk, 'DLP36904');
    }
    ...
    if (dateTimeUtils.isAfterOrEqual(dateTimeUtils.formatDate(issueDate), dateTimeUtils.formatDate('2024-03-25'))) {
        const basisActiveCodes = ['IBA3', 'IBA5', 'IBAP3', 'IBAP5', 'IBA3REINVEST', 'IBA5REINVEST', 'IBAP3VTB', 'IBAP5VTB', 'IBAV3VTB', 'IBAV5VTB', 'IBA2P3VTB', 'IBA2P5VTB', 'IBA2V3VTB', 'IBA2V5VTB'];
        if (basisActiveCodes.includes(productCode)) {
            historyIncome = basicConditions.insuranceTerms == 5 ? '0.8' : '0.15';
        }
    }
    const optionPrice = round((basicInvestmentParameters?.optionPrice ?? 1) * 100, 2);
    ...
    const discount = body?.basicInvestmentParameters?.discount;
    ...
    const strategyDescription = body?.basicInvestmentParameters?.investmentStrategyDescriptionFull;
    const outpaymentPlan = printoutsHelper.getOutpaymentPlan(body?.outpaymentPlan);
