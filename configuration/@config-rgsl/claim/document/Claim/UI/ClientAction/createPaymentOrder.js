...
module.exports = async function createPaymentOrder(input, ambientProperties) {
    ...
    const selectedItems = this.view.getControlByElementId('beneficiariesTable').dataSource.selectionModel.selected();
    ...
    const beneficiaries = input.rootContext.Body.claimBeneficiaries;
    ...
    if (existingPaymentOrders && selectedItems.length !== 0) {
        ...
        const existing = existingPaymentOrders
            .find(item => item.beneficiaryCode === beneficiary.partyCode &&
                item.paymentOrderNumber === beneficiary.assignedPaymentOrderNumber);

        if (existing) {

            ambientProperties.services.confirmationDialog
                .showConfirmation(`Для указанного выгодоприобретателя уже создан РНВ: ${existing.paymentOrderNumber}`, 'OK', 'OK', 2);
            return;
        }
    }
    ...
    const request = {
        ...
        data: {
            data: {
                ...
                beneficiaryReasonCode: beneficiary.beneficiaryReason.code
            }
        },
    ...
    if (isCreated) {
        ...
        const poRecord = { paymentOrderNumber: poNumber, beneficiaryCode: beneficiary.partyCode };
        ...
        beneficiary.assignedPaymentOrderNumber = poRecord.paymentOrderNumber;
