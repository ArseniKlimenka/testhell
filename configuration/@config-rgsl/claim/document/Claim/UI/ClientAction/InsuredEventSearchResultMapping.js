...
module.exports = async function InsuredEventSearchResultMapping(input, ambientProperties) {
    ...
    if (lookupSelection[0] && lookupSelection[0].resultData) {
        ...
        if (risksResult && risksResult.data && risksResult.data.length > 0) {
        ...
        const insuredSumByPeriod = risksToAdd.map(risk => {
        ...
        input.data.Body.mainAttributes.risksInsuredSumByPeriod = insuredSumByPeriod;
        ...
        input.data.Body.claimBeneficiaries.length = 0;
    }
