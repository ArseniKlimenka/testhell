...
const { product, partnerCode } = require('@config-rgsl/infrastructure/lib/lifeInsuranceConstants');
...
module.exports = function mapping(input) {
    ...
    const isPolicyHolder = input.body.insuredPerson.isPolicyHolder || product.ECOF2ZENIT == productCode;
    const holderInfo = printoutsHelper.getNoticeBFKO(holder.email, holder.phoneNumber, input.body.basicConditions.issueDate);
    const holder60plus = printoutsHelper.getAge(input.body.policyHolder.partyData.partyBody.partyPersonData?.dateOfBirth, input.body.basicConditions.issueDate) && product.ECOF2ZENIT != productCode;
    ...
    const isECOFVTB = [product.ECOFPVTB, product.ECOFVVTB, product.ECOF2ZENIT].includes(productCode);
    const isECOFVTBDMS = [product.ECOFPVTB, product.ECOFVVTB].includes(productCode);
    const DMS10 = [product.ECOF2ZENIT].includes(productCode);

    const isVTBpartner = input.body.mainInsuranceConditions.partner.partnerBusinessCode == partnerCode.VTB;
    const isHideHolderDataAgreement = [product.ECOFPVTB, product.ECOFVVTB, product.ECOF2ZENIT].includes(productCode);
    const isHideFinServiceNotify = [product.ECOFPVTB, product.ECOFVVTB, product.ECOF2ZENIT].includes(productCode);
