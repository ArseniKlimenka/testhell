...
module.exports = function appendixRule(input) {
    ...
    // Печатка: Помощь при потере работы
    if (additionalServices.some(item => item.serviceCode.includes('LEG4', 'LEG5', 'LEG6'))) {
        appendix.push({
            name: `CommonAppendixImageContainer/img/LEG.pdf`,
            mode: 'Append'
        });
    }

    // Образовательный Консьерж
    if (additionalServices.some(item => item.serviceCode == "PsychologicalHelp"))
    { appendix.push({
        name: `CommonAppendixImageContainer/img/PsychologicalHelp.pdf`,
        mode: 'Append'
    }); }
