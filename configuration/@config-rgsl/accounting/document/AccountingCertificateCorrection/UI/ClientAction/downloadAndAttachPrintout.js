...
/**
 * @translationKey {translationKey} SaveDocPlease
...
module.exports = async function downloadAndAttachPrintout(input, ambientProperties) {
    ...
    if (!input.context.OriginalDocumentNumber) {

        const msg = translate(ambientProperties.configurationCodeName.toUpperCase(), messages.SaveDocPlease);
        ambientProperties.services.confirmationDialog.showWarning(msg, messages.OK, messages.Cancel, smallSize);
        return;
    }
    ...
    const request = {
        ...
        data: {
            data: {
                ...
                correctionNumber: input.context?.Body?.correctionNumber,
