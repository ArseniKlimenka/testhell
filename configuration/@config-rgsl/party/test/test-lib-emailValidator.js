...
describe('Tests for EmailValidator', function() {
    const emailValidationRule = require('@config-rgsl/party/lib/e-mailValidator');

    it('Should Be Valid', function() {
        validSupported.forEach(email => {
            expect(emailValidationRule(email)).to.equal(true, `Email validation for '${email}' should pass!`);
        });
    });

    it('Should Be Invalid', function() {
        invalidSupported.forEach(email => {
            expect(emailValidationRule(email)).to.equal(false, `Email validation for '${email}' should fail!`);
        });
    });

    it('Should Be Invalid (UnSupported By the emailValidator)', function() {
