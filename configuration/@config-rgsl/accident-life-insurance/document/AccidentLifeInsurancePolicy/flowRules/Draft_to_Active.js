...
const { paymentFrequency, productGroupArray } = require('@config-rgsl/infrastructure/lib/lifeInsuranceConstants');
...
module.exports = function rule(input) {
    ...
    const hideForAccidentPro = productGroupArray.MEDPRO;
    ...
    if (!isSkipAttachmentsValidationAPI) {
        ...
        if (!isEPolicy && !isOffer && !attachmentsPackage.some(item => item.entityId.toUpperCase() == contractId.toUpperCase() && item.attachmentType == 'contractSigned')) {
            validationErrors.push({
                errorCode: "Draft_to_Active_contractAttachment_contractSigned"
            });
        }

        if (!isEPolicy && BFKOPartner && !hideForAccidentPro.includes(productCode) && !attachmentsPackage.some(item => item.entityId.toUpperCase() == contractId.toUpperCase() && ((item.attachmentType == 'bankNotification') || (item.attachmentType == 'personalData' || item.attachmentType == 'noticeFinServices')))) {
            validationErrors.push({
