...
module.exports = async function partyResultMapping(input, ambientProperties) {
    ...
    if (componentContext.partyCode) {
        ...
        if (result.data.length !== 0) {
            ...
            alignPolicyHolderAndInsured(body, data, isCollectivePolicy);
...
function alignPolicyHolderAndInsured(body, data, isCollectivePolicy) {

    if (data.participantType == lifeInsuranceConstants.participantType.policyHolder || data.participantType == lifeInsuranceConstants.participantType.insuredPerson) {
        ...
        const insuredIsPolicyHolder = isCollectivePolicy ? false : body?.productConfiguration?.insuredIsPolicyHolder;

        // set isPolicyHolder in case of LegalEntity policy holder
        ...
        if (isPolicyHolder && policyHolderPartyCode != insuredPersonPartyCode) {
            ...
            if (data.participantType == lifeInsuranceConstants.participantType.policyHolder) {

                setValue(body, 'insuredPerson.partyData', body?.policyHolder?.partyData);
            }

            if (data.participantType == lifeInsuranceConstants.participantType.insuredPerson) {
