...
const {
    product, productGroupArray
} = require('@config-rgsl/infrastructure/lib/lifeInsuranceConstants');
const printoutsHelper = require("@config-rgsl/life-insurance/lib/printoutsHelper");

function finKnowledgeQuestionnaireHolderMapping(input, that) {
    ...
    const isBasisAvtivePrem2 = [product.IBA2P3].includes(productCode);
    let contentPoint = '';
    if (isBasisAvtivePrem2) {
        contentPoint = '.';
    }
    ...
    const bottomLeftContent =
        ...
        'content: "ООО СК «Росгосстрах Жизнь»' + contentPoint + ' 121059, г. Москва, вн.тер.г. муниципальный округ\\A Дорогомилово, ул. Киевская, д. 7, к. 1. Лицензии Банка России: СЖ № 3879, СЛ № 3879\\A ИНН 7743504307. ОГРН 1037739821514";' +
        '}' +
        '}' +
        '</style>';
...
module.exports = {
    finKnowledgeQuestionnaireHolderMapping
};
