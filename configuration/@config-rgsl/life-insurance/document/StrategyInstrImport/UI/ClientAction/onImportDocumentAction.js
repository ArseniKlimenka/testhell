...
module.exports = async function onImportDocumentAction(input, ambientProperties) {
    ...
    if (input.context.Number && state === 'Importing') {
        const StrategyInstrImportHandler = (message) => {
            if (message.eventType === 'StatusChanged') {
                this.unsubscribeFromEventsFromCurrentEntity('StrategyInstrImportServerSideHandler');
        ...
        this.subscribeToEventsFromCurrentEntity({
            handler: {
                name: 'StrategyInstrImportServerSideHandler',
                func: StrategyInstrImportHandler
            }
        });
    }
