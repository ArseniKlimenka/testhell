...
module.exports = function onLoadDocumentAction(input, ambientProperties) {
    ...
    if (input.context.Number && state === 'Loading') {
        const StrategyInstrLoadHandler = (message) => {
            if (message.eventType === 'StatusChanged') {
                this.unsubscribeFromEventsFromCurrentEntity('StrategyInstrLoadServerSideHandler');
        ...
        this.subscribeToEventsFromCurrentEntity({
            handler: {
                name: 'StrategyInstrLoadServerSideHandler',
                func: StrategyInstrLoadHandler
            }
        });
    }
