...
module.exports = async function onImportDocumentAction(input, ambientProperties) {
    ...
    if (input.context.Number && state === 'Importing') {
        const RateOfReturnRulesImportHandler = (message) => {
            if (message.eventType === 'StatusChanged') {
                this.unsubscribeFromEventsFromCurrentEntity('RateOfReturnRulesImportServerSideHandler');
        ...
        this.subscribeToEventsFromCurrentEntity({
            handler: {
                name: 'RateOfReturnRulesImportServerSideHandler',
                func: RateOfReturnRulesImportHandler
            }
        });
    }
