...
module.exports = function onLoadDocumentAction(input, ambientProperties) {
    ...
    if (input.context.Number && state === 'Loading') {
        const ProductConfigurationLoadHandler = (message) => {
            if (message.eventType === 'StatusChanged') {
                this.unsubscribeFromEventsFromCurrentEntity('ProductConfigurationLoadServerSideHandler');
        ...
        this.subscribeToEventsFromCurrentEntity({
            handler: {
                name: 'ProductConfigurationLoadServerSideHandler',
                func: ProductConfigurationLoadHandler
            }
        });
    }
