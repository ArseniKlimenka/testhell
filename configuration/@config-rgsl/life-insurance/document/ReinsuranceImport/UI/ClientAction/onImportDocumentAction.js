...
module.exports = async function onImportDocumentAction(input, ambientProperties) {
    ...
    if (input.context.Number && state === 'Importing') {
        const ReinsuranceImportHandler = (message) => {
            if (message.eventType === 'StatusChanged') {
                this.unsubscribeFromEventsFromCurrentEntity('ReinsuranceImportServerSideHandler');
        ...
        this.subscribeToEventsFromCurrentEntity({
            handler: {
                name: 'ReinsuranceImportServerSideHandler',
                func: ReinsuranceImportHandler
            }
        });
    }
