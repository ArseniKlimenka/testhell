...
module.exports = async function onImportDocumentAction(input, ambientProperties) {
    ...
    if (input.context.Number && state === 'Importing') {
        const InvestmentProfitImportHandler = (message) => {
            if (message.eventType === 'StatusChanged') {
                this.unsubscribeFromEventsFromCurrentEntity('InvestmentProfitImportServerSideHandler');
        ...
        this.subscribeToEventsFromCurrentEntity({
            handler: {
                name: 'InvestmentProfitImportServerSideHandler',
                func: InvestmentProfitImportHandler
            }
        });
    }
