...
module.exports = function rule(input) {
    ...
    const hideForMedPro = productGroupArray.MEDPRO;
    ...
    if (!isSkipAttachmentsValidationAPI) {
        ...
        if (!isEPolicy && BFKOPartner && !hideForMedPro.includes(productCode) && !attachmentsPackage.some(item => item.entityId.toUpperCase() == contractId.toUpperCase() && ((item.attachmentType == 'bankNotification') || (item.attachmentType == 'personalData' || item.attachmentType == 'noticeFinServices')))) {
            validationErrors.push({
