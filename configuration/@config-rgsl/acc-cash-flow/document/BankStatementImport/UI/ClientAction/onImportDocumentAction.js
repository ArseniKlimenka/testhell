...
module.exports = async function onImportDocumentAction(input, ambientProperties) {
    ...
    if (input.context.Number && state === 'Importing') {
        const bankStatementImportHandler = (message) => {
            if (message.eventType === 'StatusChanged') {
                this.unsubscribeFromEventsFromCurrentEntity('bankStatementImportServerSideHandler');
        ...
        this.subscribeToEventsFromCurrentEntity({
            handler: {
                name: 'bankStatementImportServerSideHandler',
                func: bankStatementImportHandler
            }
        });
    }
